  ...
  return (
    <section className="w-full bg-secondary/30 py-16 md:py-24">
      <div className="container px-4 md:px-6">
        <h2 className="mb-12 text-center text-sm font-semibold uppercase tracking-[0.2em] text-amber-500">{t('whyTitle')}</h2>
        <div className="grid gap-6 md:grid-cols-3">
          {CARDS.map(({ Icon, key }) => (
            <Card key={key} className="rounded-3xl border-border/50 shadow-sm">
              <CardContent className="p-6">
                <div className="mb-4 flex h-12 w-12 items-center justify-center rounded-xl bg-primary/10">
                  <Icon className="h-6 w-6 text-primary" />
                </div>
                <h3 className="mb-2 text-lg font-semibold text-blue-950">{t(`${key}Title`)}</h3>
